...
export function AdminOvertimeDashboardPage() {
  ...
  function weekBadge(totalMinutes: number) {
    ...
    if (totalMinutes >= 8 * 60) return { label: '주의', cls: 'bg-yellow-100 text-yellow-700 border border-yellow-200' }
    return { label: '양호', cls: 'bg-success-100 text-success-700 border border-success-200' }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className={`rounded-xl p-3 ${danger12Count > 0 ? 'bg-danger-50' : 'bg-gray-50'}`}>
              <div className="flex items-center gap-2 mb-1">
                <AlertTriangle className={`w-4 h-4 ${danger12Count > 0 ? 'text-danger-500' : 'text-gray-400'}`} />
                <span className={`text-xs font-medium ${danger12Count > 0 ? 'text-danger-600' : 'text-gray-500'}`}>주 12h 초과</span>
              </div>
              <p className={`text-2xl font-bold ${danger12Count > 0 ? 'text-danger-700' : 'text-gray-400'}`}>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        {/* 기준 레전드 */}
        <div className="px-4 py-2 bg-gray-50 border-b border-gray-100 flex items-center gap-2 flex-wrap text-xs text-gray-500">
          <span className="font-medium text-gray-600">초과여부 기준 (월간):</span>
          ...
          <span className="text-gray-400 ml-1">(주 8/12h 기준)</span>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  ...
                  <th className="px-4 py-3 text-right text-xs font-semibold text-blue-600 whitespace-nowrap">연장근무</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-indigo-600 whitespace-nowrap">야간근무</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-orange-600 whitespace-nowrap">휴일근로</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-red-600 whitespace-nowrap">휴일+야간</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-600 whitespace-nowrap">초과여부</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700 whitespace-nowrap">합계</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-500 whitespace-nowrap">상세</th>
                </tr>
              </thead>
                  ...
                  return (
                    <Fragment key={row.empId}>
                      ...
                      {isExpanded && row.weeklyDetails.map((w) => {
                        ...
                        return (
                          <tr key={w.week} className="bg-blue-50/40">
                            <td className="pl-6 pr-4 py-2 text-xs text-gray-500 font-medium">{w.week}주차</td>
                            <td className="px-4 py-2" />
                            <td className="px-4 py-2 text-right text-xs text-blue-500">{fmtH(w.extended)}</td>
                            <td className="px-4 py-2 text-right text-xs text-indigo-500">{fmtH(w.night)}</td>
                            <td className="px-4 py-2 text-right text-xs text-orange-500">{fmtH(w.holiday)}</td>
                            <td className="px-4 py-2 text-right text-xs text-red-500">{fmtH(w.holidayNight)}</td>
                            <td className="px-4 py-2 text-center">
                              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${wb.cls}`}>{wb.label}</span>
                            </td>
                            <td className="px-4 py-2 text-right text-xs font-bold text-gray-700">{fmtH(w.total)}</td>
                            <td />
                          </tr>
                        )
                      })}
              ...
              <tfoot>
                <tr className="border-t-2 border-gray-200 bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right font-bold text-blue-600">{fmtH(monthlyTable.reduce((s, r) => s + r.extended, 0))}</td>
                  <td className="px-4 py-3 text-right font-bold text-indigo-600">{fmtH(monthlyTable.reduce((s, r) => s + r.night, 0))}</td>
                  <td className="px-4 py-3 text-right font-bold text-orange-600">{fmtH(monthlyTable.reduce((s, r) => s + r.holiday, 0))}</td>
                  <td className="px-4 py-3 text-right font-bold text-red-600">{fmtH(monthlyTable.reduce((s, r) => s + r.holidayNight, 0))}</td>
                  <td />
                  <td className="px-4 py-3 text-right font-bold text-gray-800">{fmtH(monthlyTable.reduce((s, r) => s + r.total, 0))}</td>
                  <td />
                </tr>
              </tfoot>
            </table>
