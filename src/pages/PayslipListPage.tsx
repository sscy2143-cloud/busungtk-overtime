...
export function PayslipListPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 본인 확인 */}
      {!verified ? (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 max-w-md mx-auto text-center">
          <div className="flex justify-center mb-4">
            <div className="w-14 h-14 bg-primary-50 rounded-full flex items-center justify-center">
              <Lock size={24} className="text-primary-600" />
            </div>
          </div>
          <h2 className="text-lg font-bold text-gray-900 mb-1">본인 확인</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500">지급월</th>
                ...
                <th className="px-4 py-3 text-center text-xs font-semibold text-gray-400 w-16">보기</th>
              </tr>
            </thead>
              ...
              {payslips.map((slip, idx) => {
                ...
                return (
                  <tr key={slip.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm font-semibold text-gray-800">{y}년 {parseInt(m)}월</td>
                    <td className="px-4 py-3 text-center text-xs text-gray-500">
                      {slip.work_start && slip.work_end
                        ? `${slip.work_start.slice(5).replace('-', '/')} ~ ${slip.work_end.slice(5).replace('-', '/')}`
                        : '-'}
                    </td>
                    <td className="px-4 py-3 text-xs text-gray-500 truncate max-w-[180px]">{slip.file_name}</td>
                    <td className="px-4 py-3 text-xs text-gray-500 truncate max-w-[150px]">{slip.message ?? '-'}</td>
