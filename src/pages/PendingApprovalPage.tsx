...
export function PendingApprovalPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-dark-50 to-dark-100 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-sm text-center">
        ...
        <div className="bg-dark-50 rounded-xl px-4 py-3 mb-6">
          <p className="text-xs text-dark-400 mb-0.5">로그인 계정</p>
          <p className="text-sm font-medium text-dark-700">{user?.email ?? '-'}</p>
        </div>

        <button
          onClick={signOut}
          className="flex items-center justify-center gap-2 w-full py-3 text-sm font-medium text-dark-600 border border-dark-200 rounded-xl hover:bg-dark-50 transition-colors"
        >
          <LogOut size={16} />
          로그아웃
        </button>
